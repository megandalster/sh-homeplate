...
class testRoute extends UnitTestCase {
    function testRouteModule() {    
    	...
    	$this->assertEqual($route->get_day(), "Wednesday December 28, 2011");
	   	...
	   	$this->assertEqual($route->get_pickup_stops(), array("Food Lion - Palmetto Bay","Piggly Wiggly"));
	   	$this->assertEqual($route->get_dropoff_stops(), array());
	   	$this->assertEqual($route->get_status(), "created");
