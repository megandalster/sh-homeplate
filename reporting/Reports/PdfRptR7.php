...
require(dirname(__FILE__).'/../Traits/R7DataTrait.php');
...
class PdfRptR7 extends PdfGraphReport
{
    ...
    function run() {
        ...
        $graph->title->SetFont(FF_DEFAULT,FS_NORMAL,18);
        $graph->title->SetColor('#333');
        $graph->title->Set("Second Helpings\nDonor 6 Month Food Trend");
        $graph->legend->Pos(0.5,0.9, 'center', 'botton');
...
//        $lplot->mark->setFillColor("red");
    ...
    function newPage() {
        ...
        $this->pdf->SetXY($x,$y1);
        $this->pdf->SetFont('','B');
        $this->pdf->MultiCell(14,4,"Change\n%",1, 'C');
